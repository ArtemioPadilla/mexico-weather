...
| `e2e/mapa.spec.ts` | map + search visible; radar + legend; satellite (no legend); timeline scrubs; temperature; wind; sunlight | 7 |
...
- **NOT YET COVERED** (slice 5b ships humidity but the e2e was wind/sunlight). The pattern is loop-friendly — see existing temperature test.
...
| `mapa-11` humidity activates | ❌ |
| `mapa-12` pressure activates | ❌ |
